 ...
 */
@Data
@Schema(description = "批量创建工序模板请求参数")
public class CreateProcessTemplateBatchRequest {
    
    @Schema(description = "模板名称（同一批次使用同一个模板名称）", requiredMode = Schema.RequiredMode.REQUIRED, example = "标准模板")
    ...
    @Schema(description = "模板下的工序列表", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotEmpty(message = "工序列表不能为空")
    @Valid
    private List<Item> processes;
    
    @Data
    @Schema(description = "工序模板明细")

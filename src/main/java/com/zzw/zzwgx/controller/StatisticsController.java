...
public class StatisticsController {
    ...
    public Result<java.util.List<StatisticsResponse.ProcessTimeStat>> getProcessTimeStatistics(
        ...
        java.util.List<StatisticsResponse.ProcessTimeStat> stats = statisticsService.getProcessTimeStatisticsForCurrentUser(year, month);
        return Result.success(stats);
    }
    
    @Operation(summary = "获取进尺长度统计", description = "根据年月查询指定工点的进尺长度统计。返回该工点在指定月份的循环次数和总进尺长度（单位：米）。")
    @GetMapping("/advance-length")
    public Result<java.util.List<StatisticsResponse.AdvanceLengthStat>> getAdvanceLengthStatistics(
            @Parameter(description = "年份，4位数字", required = true, example = "2024") @RequestParam Integer year,
            @Parameter(description = "月份，1-12", required = true, example = "1") @RequestParam Integer month) {
        log.info("查询当前管理员可管理工点的进尺长度统计列表，年份: {}, 月份: {}", year, month);
        java.util.List<StatisticsResponse.AdvanceLengthStat> stats = statisticsService.getAdvanceLengthStatisticsForCurrentUser(year, month);
        return Result.success(stats);
    }
    
    @Operation(summary = "获取本周超耗总时间统计", description = "查询指定工点本周的超时和节省时间统计（管理员）。返回本周所有已完成工序的超时总时间和节省总时间（单位：小时）。")
    @GetMapping("/overtime-week")
    public Result<java.util.List<StatisticsResponse.OvertimeStat>> getOvertimeStatistics() {
        log.info("查询当前管理员可管理工点的本周超耗统计列表");
        java.util.List<StatisticsResponse.OvertimeStat> stats = statisticsService.getOvertimeStatisticsForCurrentUser();
        return Result.success(stats);
    }
